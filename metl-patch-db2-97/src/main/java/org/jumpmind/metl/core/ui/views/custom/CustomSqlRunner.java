...
public class CustomSqlRunner extends Thread {

    protected static final Logger log = LoggerFactory.getLogger(CustomSqlRunner.class);

    private CustomSqlExplorer explorer;

    private static List<CustomSqlRunner> sqlRunners = new ArrayList<CustomSqlRunner>();

    private CustomSqlRunner.ISqlRunnerListener listener;
    ...
    public static List<CustomSqlRunner> getSqlRunners() {
        return sqlRunners;
    }

    public CustomSqlRunner(String sqlText, boolean runAsScript, String user, IDb db, Settings settings) {
        this(sqlText, runAsScript, user, db, settings, null, null);
    }

    public CustomSqlRunner(String sqlText, boolean runAsScript, String user, IDb db, Settings settings, ISqlRunnerListener listener) {
        this(sqlText, runAsScript, user, db, settings, null, listener);
    }

    public CustomSqlRunner(String sqlText, boolean runAsScript, String user, IDb db, Settings settings, CustomSqlExplorer explorer) {
        this(sqlText, runAsScript, user, db, settings, explorer, null);
    }

    public CustomSqlRunner(String sqlText, boolean runAsScript, String user, IDb db, Settings settings, CustomSqlExplorer explorer, ISqlRunnerListener listener) {
        this(sqlText, runAsScript, user, db, settings, explorer, listener, null, false);
    }

    public CustomSqlRunner(String sqlText, boolean runAsScript, String user, IDb db, Settings settings, CustomQueryPanel queryPanel, boolean isInQueryGeneralResults) {
        this(sqlText, runAsScript, user, db, settings, null, null, queryPanel, isInQueryGeneralResults);
    }

    public CustomSqlRunner(String sqlText, boolean runAsScript, String user, IDb db, Settings settings, CustomSqlExplorer explorer, ISqlRunnerListener listener, CustomQueryPanel queryPanel, boolean isInQueryGeneralResults) {
        ...
        this.autoCommit = settings.getProperties().is(Settings.SQL_EXPLORER_AUTO_COMMIT);
        this.user = user;
    ...
    public void run() {
        ...
        boolean resultsAsText = properties.is(Settings.SQL_EXPLORER_RESULT_AS_TEXT);
        int maxResultsSize = properties.getInt(Settings.SQL_EXPLORER_MAX_RESULTS);
        String delimiter = properties.get(Settings.SQL_EXPLORER_DELIMITER);
        boolean ignoreWhenRunAsScript = properties.is(Settings.SQL_EXPLORER_IGNORE_ERRORS_WHEN_RUNNING_SCRIPTS);
                        ...
                        while (hasResults || updateCount != -1 || firstTimeThrough) {
                            ...
                            try {
                                if (hasResults) {
                                    ...
                                    if (!runAsScript) {
                                        if (!resultsAsText) {
                                            resultComponents.add(new CustomTabularResultLayout(explorer, db, sql, rs, listener, user, settings, queryPanel, showSqlOnResults, isInQueryGeneralResults));
                                        } else {
    ...
    public interface ISqlRunnerListener extends Serializable {
